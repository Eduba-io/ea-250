...
Bespoke, single-page site prepared for Propy ahead of the conference. Headline opens with a read on where AI actually fits in Propy's closing stack. The page carries the full pitch body from `fit-analysis.md` section 7 in Propy's own visual vocabulary: Playfair Display for display type, Space Grotesk for body, the signature `#38A6FB` sky-blue accent against a clean light neutral, and the single-color logomark in the nav. Eduba shows up only in the footer. The CTA routes straight to Matt Creamer's calendar.
